...
export default class Agent {
    ...
    public avoidenceWeight_: number = 0.5;
    public simulator: Simulator = null
    private newVelocity_: Vector2 = new Vector2(0, 0);

    public update() {
        this.velocity_ = this.newVelocity_;
        let v2 = Vector2.addition(this.position_, Vector2.multiply2(this.simulator.timeStep_, this.velocity_));
        this.position_ = v2;
        if (isNaN(this.position_.x) || isNaN(this.position_.y)) {
    ...
    public insertAgentNeighbor(agent: Agent, rangeSq: ObserverObj<number>) {
        ...
        if (agent && this != agent && this.type == agent.type) {
    ...
    public computeNeighbors() {
        ...
        if (this.maxNeighbors_ > 0) {
            ...
            this.simulator.kdTree_.computeAgentNeighbors(this, obserObj);
        }
    }

    /** 基于当前邻居和障碍物信息，计算智能体的新速度。 */
    public computeNewVelocity() {
        ...
        for (let i = 0; i < this.agentNeighbors_.length; ++i) {
            ...
            if (distSq > combinedRadiusSq) {
                ...
                let invTimeStep = 1 / this.simulator.timeStep_;
            ...
            line.point = Vector2.addition(this.velocity_, Vector2.multiply2(this.avoidenceWeight_, u));
    ...
    * ORCA算法的一部分，用于解决速度优化问题。它们通过线性规划来计算智能体在满足所有避碰约束的情况下能够达到的最优速度。
    */
